...
expected_log = "ZJIT: hot method detected (idx=1)\n"   # Stage 3d.3: idx=0 は builtin Array#length が占有
assert_eq(out, "0\n",        "閾値ちょうど: 出力は変わらない")
...
assert_eq(out, "55\n",       "fib(10) 結果は不変")
assert_eq(err, expected_log, "fib(10) で fib (idx=1) が hot 検出")
...
expected_multi = "ZJIT: hot method detected (idx=1)\nZJIT: hot method detected (idx=2)\n"   # Stage 3d.3: builtin Array#length が idx=0 を占有
assert_eq(out, "0\n",          "複数メソッド: 出力は変わらない")
...
out, err = run_capture(fib_src)
assert_eq(out, "55\n",                                 "JIT-2 OFF: fib(10) 結果")
assert_eq(err, "ZJIT: hot method detected (idx=1)\n",  "JIT-2 OFF: HIR ダンプなし (Stage 3d.3 で idx=1)")
...
assert_includes(err, "ZJIT: hot method detected (idx=1)", "JIT-2 ON: hot 検出ログ")
assert_includes(err, "ZJIT HIR (raw) for method idx=1:",        "JIT-2 ON: HIR ヘッダ")
...
assert_includes(err, "Call m1(",                          "JIT-2 ON: 再帰呼び出し (Stage 3d.3 で fib は m1)")
...
assert_includes(err, "ZJIT HIR (raw) for method idx=1:", "JIT-2 ON: tarai HIR ヘッダ")
...
assert_eq(out, "55\n", "JIT-3b2: fib(10) 結果不変")
assert_includes(cfg_section, "ZJIT CFG analysis for method idx=1:", "JIT-3b2: 分析ヘッダ")
...
assert_eq(out, "55\n", "JIT-4: fib(10) 結果不変")
assert_includes(lir, "ZJIT LIR for method idx=1:", "JIT-4: LIR ヘッダ")
# arm64 主要命令が出る
...
assert_includes(lir, "bl m1",      "JIT-4: 再帰呼び出し (Stage 3d.3 で fib は m1)")
